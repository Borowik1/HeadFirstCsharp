...
namespace _08___Retired_Jersey_Numbers
{
    public partial class Form1 : Form
    {
        ...
        {
            InitializeComponent();
            foreach (int item in retiredNumbers.Keys)
            {
                numbersComboBox.Items.Add(item);
            }
            numbersComboBox.SelectedIndex = 0;
        }

        private void numbersComboBox_SelectedIndexChanged(object sender, EventArgs e)
        {
            nameTextBox.Text = retiredNumbers[(int)numbersComboBox.SelectedItem].Player;
            yearTextBox.Text = retiredNumbers[(int)numbersComboBox.SelectedItem].YearRetired.ToString();
        }
    }
}
